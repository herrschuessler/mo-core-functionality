...
function attach_style( $module ) {
	...
	// use minified scripts if not in dev mode
	$folder = ( defined( 'WP_ENV' ) && WP_ENV === 'development' ) ? 'dev/' : 'dist/';

	// Script version = theme version
	$version = wp_get_theme()['Version'];

	$path = get_template_directory_uri() . '/assets/css/' . $folder . filter_var( $module, FILTER_SANITIZE_ENCODED ) . '.css?version=' . $version;
